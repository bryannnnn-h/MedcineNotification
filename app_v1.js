...
app.get('/get_contact_data', (req, res)=>{
  ...
  var result1_length
  var result2_length
  var bin = '<img src="https://luffy.ee.ncku.edu.tw:5331/img/bin.png">'//trashcan icon
  var str1 = ''
  var str2 = ''
    ...
    for(var i=0; i<all_supervisor.length; i=i+1){
      connection.query(`SELECT * FROM user_Info WHERE userId = '${all_supervisor[i]['supervisorId']}'`, (err, supervisor_result) => {
        for(var j=0; j<supervisor_result.length; j=j+1){
          str1 = str1+`<img src="${supervisor_result[j]['picture']}"><h1>${supervisor_result[j]['userName']}</h1>${bin}<button class="btn" onclick="check_to_delete_supervisor('${supervisor_result[j]['userId']}')">刪除</button><br>`//+`<button id="btnShare_beSupervisor" >新增我的提醒人</button>`
        }
      })
    }
    setTimeout(function(){str1 = str1+`<button id="btnShare_beSupervisor" >新增我的提醒人</button>`},200)
  })
    ...
    for(var i=0; i<all_supervisee.length; i=i+1){
      connection.query(`SELECT * FROM user_Info WHERE userId = '${all_supervisee[i]['superviseeId']}'`, (err, supervisee_result) => {
        for(var j=0; j<supervisee_result.length; j=j+1){
          //<div>
          str2 = str2+`<img src="${supervisee_result[j]['picture']}"><h1>${supervisee_result[j]['userName']}</h1>${bin}<button class="btn" onclick="check_to_delete_supervisee('${supervisee_result[j]['userId']}')">刪除</button><br>`//+`<button id="btnShare_beSupervisor" >新增我的被監督人</button>`
          //</div>
        }
      })
    }
    setTimeout(function(){str2 = str2+`<button id="btnShare_beSupervisee" >新增我的被監督人</button>`},200)
  })

  setTimeout(function(){
    console.log(str1)
    console.log(result1_length)
    console.log(str2)
    console.log(result2_length)
  }, 250)//資料庫需要至少0.1秒存取QQ
...
app.get('/delete_contact', (req,res)=>{
  ...
  var result1_length
  var result2_length
  var bin = '<img src="https://luffy.ee.ncku.edu.tw:5331/img/bin.png">'
  var str1 = ''
  var str2 = ''
